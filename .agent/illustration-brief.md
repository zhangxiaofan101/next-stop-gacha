...
**前置（batch 0，一次性）**：用户终审 M43 三样张（`raw/journal/dest-{hangzhou|dunhuang|sanya}-v{1,2}.png` 六选三）→ cc 转 q90 webp 入 `picked/dest/` 成为**基准三张**并把风格锁冻结进 design。基准未定前不开 batch 1。开批时把三样张原件（连同落选版）挪入 `raw/dest/`，raw/journal/ 自此只放手帐皮肤件。

**恒定规则（每张都适用）**：

- **风格锁**：本文件顶部「风格锁」水彩前缀一字不改；每张生成以 `picked/dest/` 基准三张为 image reference
