...
**线上**：`travel.xiaofan.me`（2026-07-26 起，见 M82；旧地址 `lab.medspiral.com/next-stop-gacha/` 只余搬家页与 308）——Vite+TS 工程化版，295 城 + 53 线（348 条）全链路（筛选/扭蛋/对比/行程/路书/足迹地图/天气）；出发地双视角（上海基座/北京，全站数据视角 + 距离排序）；后端短链分享（KV）+ 同步码云同步（Durable Object）；主题皮肤机制 + 原味（cream，原名奶油）/山水/青花/涂鸦四肤（默认 `ink`）；目的地插画共享集 295/295 全量上线。 [cc][codex]
...
**七期（2026-07-24 用户反馈插队重排；2026-07-27 广州批再插队）**：M78 ✅ → M79 ✅（M81–M84 维护/搬域名/PWA 图标插件亦 ✅）→ **M22·广州批（在做）** → M72 装饰件体系 → M70 PWA（见 🔜）；远期池见 🗄。新编号自 M85 起。 [cc]
...
1. 【内容】M22·广州批 — 第三出发地：广州 + 珠三角周边短途卡 [R2 · S3] → opus 舰队（A 批 7 分片存量视角 + B 批 4 组周边新卡调研） ｜ 2026-07-27 用户点名插队（「像北京一样，加个从广州出发」）；沿北京批全套先例执行：cc 拟候选名单只作起点、调研可增删、值得住红线淘汰、noair/norail/slowrail 逐地独立查证附依据；广州 350km 密度 14（对照北京批前 15→批后 28），补卡空间同量级；spec 同 M22 [cc]
2. 【画面+前端】M72 — 装饰件体系：增补与编排 [R2 · S2 · 🌫️] ｜ 2026-07-22 用户点名（原七期头名，两次顺延）：各皮肤装饰件大小参差、还可能加件，要系统化「加什么、怎么排好看」——每肤可不同排法、可上弹层悬窗；开工时逐肤出 mock 拍板；spec 见 design M72 [cc]
3. 【前端】M70 — 手机 App 化第一步：PWA [R2 · S2 · 🌫️] ｜ 2026-07-21 用户新增方向（App 化从 PWA 起步）；安装身份与主屏图标已由 M84 提前覆盖，本模块剩 Service Worker 离线缓存/更新策略/安装引导；开工前 scoping 拍板；spec 见 design M70 [cc]

- 七期 review gate：M72/M70 均 🌫️、开工先 AskUserQuestion；M78/M79/M81 落地后建议跨家族 review 一轮再进 M72（尚未执行）；M22·广州批为内容批，落地后按 content-checklist 交另一族抽查；gate 排法（合并轮/分轮）届时定。 [cc]
