...
**七期（2026-07-24 用户反馈插队重排）**：M78 筛选收粗 → M79 路书地图 → M72 装饰件体系 → M70 PWA（见 🔜）；远期池见 🗄。新编号自 M81 起。 [cc]
...
1. 【前端】M78 — 容忍型筛选收粗 [R2 · S2] → sonnet · high ｜ 2026-07-24 朋友实测反馈立项（选项太具象、该用包含关系）；同日拍板口径：容忍型三组收粗+不限显性化+顶档裁撤，偏好型组不动（cc 判断：反馈对容忍型成立、对偏好型不成立——天数按 design 自身偏好/容忍判据本就该归容忍型）；spec 见 design M78 [cc]
2. 【前端】M79 — 路书路线图与环线最优提示 [R2 · S2] → sonnet · high ｜ 2026-07-24 朋友实测反馈立项（想要带地图路书、环线怕平白绕路）；同日拍板：整线当原子块、地图放路书头部；spec 见 design M79 [cc]
3. 【画面+前端】M72 — 装饰件体系：增补与编排 [R2 · S2 · 🌫️] ｜ 2026-07-22 用户点名（原七期头名，2026-07-24 顺延）：各皮肤装饰件大小参差、还可能加件，要系统化「加什么、怎么排好看」——每肤可不同排法、可上弹层悬窗；开工时逐肤出 mock 拍板；spec 见 design M72 [cc]
4. 【前端】M70 — 手机 App 化第一步：PWA [R2 · S2 · 🌫️] ｜ 2026-07-21 用户新增方向（App 化从 PWA 起步），2026-07-24 顺延至第四位；开工前 scoping 拍板（SW 缓存/更新策略、安装引导位）；spec 见 design M70 [cc]

- 七期 review gate：M72/M70 均 🌫️、开工先 AskUserQuestion；M78/M79 落地后建议跨家族 review 一轮再进 M72；gate 排法（合并轮/分轮）届时定。 [cc]
