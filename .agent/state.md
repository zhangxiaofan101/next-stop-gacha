...
- **M83 老存档跨域名接运走哪条路**（M82 落地后遗留，2026-07-26）：`nextstop_v2` 在旧 origin 的 localStorage 里，新域名读不到，裸 308 过去的老玩家会看见空存档。既有同步码（Durable Object，`?sc=`）是 origin 无关的现成通道。两条路见 design M83：①旧路径先落一张极简搬家页自动接运；②只提示用户手动走一次同步码。①对用户省事但多一层要维护的页面，且和「旧地址不提供内容」有张力（那张页面本身是内容——不过只有一句话和一个按钮，构不成可 join 的正文）。待用户拍板 [cc]
- **部署时机**（2026-07-26）：M82 的代码就绪但**尚未部署**。`wrangler deploy` 会同时创建 `travel.xiaofan.me` 的 Custom Domain 并让旧地址转 308——两件事同一次生效，没有中间态。跨仓库顺序（见 orbit 仓库 design「跨仓库依赖」）：travel 与 lab 的新域名都上线并验证之后，才轮到旧域名整域 308，最后才改学术主站的入口链接 [cc]
