...
// M52 起；M61 从 cream/ink 双文件泛化到全部注册皮肤——第三套皮肤加入后仍只两两比较会漏出
// 「porcelain 缺某 token，但恰好没人比较到它」的盲区。
describe("皮肤 token 多文件漂移钉子", () => {
  const tokenNames = (rel: string) =>
    new Set([...readFileSync(join(ROOT, rel), "utf8").matchAll(/^\s*(--[\w-]+)\s*:/gm)].map(m => m[1]));
  it("每套已注册皮肤（SKINS，见 src/skins/registry.ts）的 src/skins/<id>.css 定义的 token 名集合完全一致（漏一个=某皮肤下该值静默失效）", () => {
    const bySkin = SKINS.map(s => ({ id: s.id, tokens: tokenNames(`src/skins/${s.id}.css`) }));
    const union = new Set(bySkin.flatMap(s => [...s.tokens]));
    bySkin.forEach(({ id, tokens }) => {
      const missing = [...union].filter(n => !tokens.has(n));
      expect(missing, `${id}.css 缺这些 token：${missing.join(", ")}`).toEqual([]);
    });
  });
});
