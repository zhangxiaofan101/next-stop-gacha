...
`illustSrc(assetDir, slot)` 用同一套槽位名拼路径，见 src/skins/illustrations.ts）：
  ...
  ink-decor-<name>.webp    → decor-<name>.webp  （自由装饰件，画幅不拘——design「装饰位画布契约」
                                                  明确排除在布局槽位之外，保留母版原生长宽比，只按
                                                  长边等比缩放封顶，不做目标比例校验，见 DECOR_NATIVE_MAX_DIM）
...
BANNER = (960, 480, BUDGET_DECOR)  # 题头 / 图位垫底母版均 1024x512（2:1）
DEST = (640, 427, BUDGET_DEST)     # 目的地母版 1536x1024（3:2）
# 自由装饰件（design「装饰位画布契约」：「画幅比不拘」，不属于布局槽位）：target_w/target_h 用
# None 标记「保留母版原生比例」，encode() 据此跳过比例校验、按长边等比缩放封顶——不同皮肤的
# decor 母版画幅可以各不相同（如 porcelain 缠枝莲角饰近 1:1、云纹浪纹近 2:1），不强求统一画布。
DECOR_NATIVE_MAX_DIM = 1024
DECOR = (None, None, BUDGET_DECOR)
...
def encode(src: str, dst: str, target_w: int, target_h: int, budget: int, violations: list) -> int:
    ...
    if target_w is None or target_h is None:
        # 自由装饰件（DECOR 哨兵）：不校验比例，长边封顶等比缩放（从不放大，母版本就够大）
        scale = min(1.0, DECOR_NATIVE_MAX_DIM / max(src_w, src_h))
        target_w, target_h = round(src_w * scale), round(src_h * scale)
        return _encode_resize(src, dst, target_w, target_h, budget, violations)
...
def _encode_resize(src: str, dst: str, target_w: int, target_h: int, budget: int, violations: list) -> int:
    """比例校验（或 DECOR 的原生比例封顶）之后共用的部分：resize+质量下探直到达预算。"""
...
def process_skin_dir(skin: str, violations: list):
    ...
    for f in files:
        ...
        # DECOR 哨兵（tw/th=None）：实际输出尺寸由 encode() 内部按原生比例算出，这里读回真实值
        # 供人工核对，不再打印占位的 "NonexNone"。
        dims = source_size(out_path) if tw is None else (tw, th)
        print(f"  {status} {out_name}：{size / 1024:.1f}KB（{dims[0]}x{dims[1]}，预算 {budget // 1024}KB）")
